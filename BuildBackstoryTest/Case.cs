...
namespace BuildBackstoryTest
{
    public class Case
    {
        ...
        private string AdjustString(string str, Player player, List<Player> party)
        {
            ...
            {
                Tag tag = player.Tags
                    .Where(t => HasTag(t, RequiredTags))
                    .FirstOrDefault(t => t is RelationTag);
                if (tag != null)
                {
                    res = res.Replace(RelatedPlayer, ((RelationTag)tag).RelatesTo.Name);
                }
                else
                {
                    foreach (Outcome outcome in outcomes)
                    {
                        tag = outcome.Tags.FirstOrDefault(t => t is RelationTag);
                        if (tag != null)
                        {
                            res = res.Replace(RelatedPlayer, ((RelationTag)tag).RelatesTo.Name);
                            break;
